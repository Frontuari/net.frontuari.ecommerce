                ...
                <div class="panel-body">
                    <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="interger" name="price" class="form-control" value="{{ $product->price }}"
                            ...
                            <select name="stores_id" class="form-control select2">
                                @foreach ($stores as $store)
                                    <option value="{{ $store->id }}"
                                        {{ $store->id == $product->stores_id ? 'selected' : '' }}>{{ $store->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <input type="integer" name="keyword" value="{{ $product->keyword }}" class="form-control"
                                placeholder="Ejemplo: salsa, liquido, rojo, vidrio">
                        </div>
                        <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <select name="taxes[]" class="form-control select2">
                                    @foreach($taxes as $tax)
                                        <option value="{{ $tax->id }}" 
                                            {{ in_array($tax->id, $product->taxes->pluck('id')->toArray()) ? 'selected' : '' }}>
                                            {{ $tax->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group">
                            ...
                            <div id="imagePreviewContainer" class="mt-2">
                                @if ($product->photo)
                                    <img src="{{ $product->photo }}" alt="Producto Imagen" class="img-fluid" width="250px"
                                        height="250px">
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="qty_avaliable" class="form-control"
                                value="{{ $product->qty_avaliable }}" placeholder="Ingrese la cantidad disponible">
                        </div>
